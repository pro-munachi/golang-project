...
import (
	...
	"github.com/pro-munachi/golang-project/helpers"
...
func GetUsers() gin.HandlerFunc{
	return func(c *gin.Context) {
		...
		groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"_id", "null"}}, {"total_count" bson.D{{"$sum", 1}}}, {"data", bson.D{"$push", "$$R"}}}}}}
		projectStage := bson.D{
			{"$project", bson.D{
				{"_d", 0},
				{"total_count", 1},
		...
		result, err := userCollection.Aggregate(ctx, mongo.Pipeline{
			matchmatchStage, groupStage, projectStage
		})
		defer cancel()
